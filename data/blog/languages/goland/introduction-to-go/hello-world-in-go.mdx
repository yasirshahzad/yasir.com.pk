...
1. **What is the purpose of `package main`?**
   - Indicates an executable program with entry point

2. **Why is the `main()` function special?**
   - It's the entry point for executable programs

3. **What happens if you don't import a package you use?**
   - Compilation error: undefined identifier

### **Intermediate Questions**
1. **Difference between `go run` and `go build`?**
   - `go run`: compiles and executes immediately
   - `go build`: creates executable file

2. **How does Go handle unused imports?**
   - Compilation error to keep code clean

3. **What is the zero value in Go?**
   - Default value assigned to variables (e.g., "" for strings, 0 for int)

### **Advanced Questions**
1. **How would you optimize a Hello World program for size?**
   - Use build flags: `-ldflags="-s -w"`

2. **Explain Go's compilation process**
   - Source → AST → SSA → Machine code

3. **How does Go's runtime work in Hello World?**
   - Goroutine scheduler, garbage collector initialization
